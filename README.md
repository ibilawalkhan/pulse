...
13. **NAT-less Fargate in public subnets.** A NAT gateway costs ~US$32/mo just to exist — more than the rest of this stack. Instead, the three Fargate services run in **public** subnets with public IPs, locked down by a security group that only allows the ALB to reach the api's port and allows all egress. This gives the worker the outbound internet access it needs (to reach monitored endpoints, SQS, SES) with **zero NAT cost**. RDS stays in **isolated private** subnets, reachable only from the service security group — never publicly. The trade-off (public IPs on tasks) is mitigated entirely by the security groups; it's the standard free-tier-friendly pattern and, stating *why* NAT was avoided is the stronger interview answer. Terraform state lives in S3 with a DynamoDB lock; deploys assume a scoped IAM role via GitHub OIDC (no stored AWS keys), build SHA-tagged images, `terraform apply`, run Prisma migrations as a one-off ECS task, then smoke-test `/health`.
